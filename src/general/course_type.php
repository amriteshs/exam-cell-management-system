...
$TBS->LoadTemplate('course_type.html');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM course_type WHERE course_type_description=:etype";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':etype', $etype);
    $stmt->execute();
    $result = $stmt->fetchAll();
    if ($stmt->rowCount() == 1) {
        $msg_err = "Course Type: $etype already exists.";
    }

    if ($msg_err == "") {
        try {
            $status_id = ecell_get_status('on');
            $sql = "INSERT INTO `course_type`(`course_type_id`, `course_type_description`, `status_value_id`, `log_id`) VALUES (NULL, :etype, :status_id, :log_id)";
            $ac_on = "Entered a course type: $etype.";
