	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (isset($_POST["enroll"])) {
			...
			if (isset($result)) {
				...
				$dob = $mydetails["dob"];
				$tdob = explode('-', $dob);
				$dob = $tdob[2] . ' ' . ucfirst(strtolower(ecell_get_month($tdob[1]))) . ', ' . $tdob[0];
				...
				$sql = $offlineConnection->prepare("SELECT media, mime from student_documents where student_id = '$studentID' and category = 'Signature' ORDER BY document_id DESC ");
				$sql->execute();
				$result = $sql->fetchAll();
				...
				if ($output_format == 'pdf') {
					...
					$address = "Deoghat - Jhalwa, Allahabad - 211012";
					...
					h2 { line-height: 16px;}
						...
						<img src='data: $photomime;base64,$photopic'  width='130' height='160' alt='Please paste passport size photo'>
						</div>
					...
					$html .= "<tr><td  height='80px'></td><td></td></tr>  </table> </body> </html>";
					...
					$html2 = "<html>
						...
						<table style='font-size: 14px; margin-bottom: 15px'> 
						<tr><td  height='30px' ></td><td>$parent_title</td></tr>";

					foreach ($print_details_parents as $f) {
						$html2 .= "<tr><td>$i .   </td><td style='width: 220px;'>" . $f['key'] . "</td><td style='width: 62%;border-bottom: 1px dotted black;'>" . $f['value'] . "</td></tr>";
						$i++;
					}

					$html2 .= "<tr><td  height='340px'></td><td></td></tr>  </table> </body> </html>";

					$Qualification_title = "<u><b>QUALIFICATION DETAILS</b></u>";
					...
					$html3 = "<html>
						...
						<table style='font-size: 14px; margin-bottom: 15px'> <tr><td  height='30px'></td><td>$Qualification_title</td></tr>";
					foreach ($print_details_qualification as $f) {
						$html3 .= "<tr><td>$i .   </td><td style='width: 220px;'>" . $f['key'] . "</td><td style='width: 62%;border-bottom: 1px dotted black;'>" . $f['value'] . "</td></tr>";
						$i++;
					}
...
\\usepackage[a4paper,left=0.7in,right=0.7in,top=0.7in,bottom=0.8in]{geometry}
...
\\newcommand\\FrontsidePic{
	\\put(-4,0){
		\\parbox[b][\\paperheight]{\\paperwidth}{%
			\\vfill
			\\centering
			\\includegraphics[width=\\paperwidth,height=\\paperheight]{/home/ecell/apache_files/cert/orig/frame-std-details.pdf}
			\\vfill
		}
	}
}

\\newcommand{\\longunderline}[1]{\\textcolor{brown}{\\dotuline{\\textcolor{black}{#1}\\hfill\\mbox{}}}}
...
\\begin{document}
	...
	\\begin{figure}[H]
		...
		\\vspace{35mm}
			\\begin{tabular}[!bt]{ p{12cm} p{2cm}}
				\\begin{tabular}{ p{3.2cm} p{9cm} }
					\\hspace{-3mm} \\textbf{Student Name:} & $student_name \\\\
					\\hspace{-3mm} \\textbf{Enrollment no.:} & $enroll \\\\
		...
		\\begin{minipage}[]{.12\\textwidth}
			...
			\\includegraphics[width=\\textwidth,height=7mm,keepaspectratio]{/home/ecell/apache_files/cert/$enroll/$sign_file_name}
		\\end{minipage}\\hfill
	\\end{figure}
...
";
	...
	\\begin{longtable}{p{2.0in} p{0.05in} p{4.25in}}
		\\textbf{Place:} &  & \\textbf{Signature of the Candidate:} \\\\
		& & \\\\
		\\textbf{Date:} &  & \\textbf{Signature of Father/Mother/Guardian:} \\\\
	\\end{longtable}

	\\vspace{3mm} \\hrule \\vspace{0.5mm} \\hrule \\vspace{4mm}
...
";
					...
					$tex .= "
	\\begin{longtable}{p{4.05in} p{0.05in} p{2.2in}}
		\\textbf{Name of the staff:} $staff_name &  & \\\\
